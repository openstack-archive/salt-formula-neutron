...
if [ "$1" == "server" ]; then
    ...
    su glance --shell=/bin/sh -c '/usr/bin/neutron-server --config-file=/etc/neutron/neutron.conf --config-file=/etc/neutron/plugins/opencontrail/ContrailPlugin.ini'
elif [ "$1" == "lbaas-agent" ]; then
    echo "starting neutron-lbaas-agent"
    su glance --shell=/bin/sh -c '/usr/bin/neutron-lbaas-agent --config-file=/etc/neutron/neutron.conf'
else
